...
DEFAULT_OUTPUT_DIR = str(Path(FAST_OUTPUT_ROOT) / "Model_V4_reliability_predictions")
...
def main():
    parser = argparse.ArgumentParser(
        description="Visualize conditioned Model_V4 soft LiDAR-reliability predictions."
    )
    ...
    if args.sample is not None:
        predict_one(
            ...
            device,
            Path(args.sample),
            output_dir / f"{Path(args.sample).stem}_Model_V4_reliability_overlay.png",
            threshold,
            channel_mean,
            channel_std,
            args.fault_type,
    ...
    for sample_path in sample_paths:
        output_path = output_dir / f"{sample_path.stem}_Model_V4_reliability_overlay.png"
        ious.append(
            predict_one(
                model,
                device,
