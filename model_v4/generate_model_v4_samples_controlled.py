...
DEFAULT_DATASET_DIR = Path("/mnt/3D10B36523559581/Gianluca/model_v4_outputs/model_v4_dataset")
...
def main():
    parser = argparse.ArgumentParser(
        description="Generate Model V4 soft-reliability samples in controlled resumable chunks."
    )
    ...
    for start_index in range(args.start_index, args.target_samples, args.chunk_size):
        ...
        command = [
            args.python,
            str(repo_dir / "model_v4" / "rewrite_model_v4_samples.py"),
            "--data-root",
            str(data_root),
            "--dataset-dir",
            str(dataset_dir),
