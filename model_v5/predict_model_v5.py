...
DEFAULT_MODEL_PATH = str(Path(FAST_OUTPUT_ROOT) / "models" / "Model_V5.pt")
DEFAULT_SAMPLE_DIR = str(Path(FAST_OUTPUT_ROOT) / "autoencoder_dataset")
DEFAULT_OUTPUT_DIR = str(Path(FAST_OUTPUT_ROOT) / "Model_V5_reliability_predictions")
...
def load_sample(path: Path):
    with np.load(path) as data:
        ...
        soft_target = data["target"].astype(np.float32)
        if "binary_target" in data.files:
            actual_mask = data["binary_target"].astype(np.float32)
        else:
            actual_mask = (soft_target >= 0.5).astype(np.float32)
        metadata = {}
        if "metadata_json" in data.files:
            metadata = json.loads(str(data["metadata_json"].item()))
    return faulty, clean, soft_target, actual_mask, metadata
...
def make_four_panel(clean, faulty, probability, overlay, fault_type: str) -> np.ndarray:
    ...
    probability_panel = add_panel_title(probability_heatmap(probability), "Predicted LiDAR unreliability")
...
def predict_one(
    ...
    faulty, clean, soft_target, actual_mask, metadata = load_sample(sample_path)
    ...
    overlay = add_overlay_text(
        ...
        [
            ...
            f"unreliability threshold: {threshold:.3f}",
            f"IoU: {iou:.3f}",
            f"soft target mean: {float(np.mean(soft_target)):.3f}",
        ],
    )
    ...
    probability_path = output_path.with_name(f"{output_path.stem}_unreliability_probability.png")
    ...
    print(f"Output unreliability heatmap: {probability_path}")
    print(f"IoU: {iou:.4f}")
...
def main():
    parser = argparse.ArgumentParser(
        description="Visualize conditioned Model_V5 soft LiDAR-reliability predictions."
    )
    ...
    threshold = args.threshold if args.threshold is not None else checkpoint.get("threshold", 0.65)
    ...
    if args.sample is not None:
        predict_one(
            ...
            device,
            Path(args.sample),
            output_dir / f"{Path(args.sample).stem}_Model_V5_reliability_overlay.png",
            threshold,
            channel_mean,
            channel_std,
            args.fault_type,
    ...
    for sample_path in sample_paths:
        output_path = output_dir / f"{sample_path.stem}_Model_V5_reliability_overlay.png"
        ious.append(
            predict_one(
                model,
                device,
    ...
    print(f"Wrote {len(ious)} reliability overlay images to {output_dir}")
