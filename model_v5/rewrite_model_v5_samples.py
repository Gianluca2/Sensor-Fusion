...
DEFAULT_OUTPUT_DIR = Path("/mnt/3D10B36523559581/Gianluca/model_v5_outputs/model_v5_dataset")
...
def stack_layers(bev_layers: dict) -> np.ndarray:
    return np.stack([bev_layers[layer].astype(np.float32) for layer in V5_LAYERS], axis=0)
...
def add_radar_expected_lidar_density_residual(bev_layers: dict) -> None:
    radar_density = np.clip(bev_layers["radar_density"], 0.0, 1.0)
    lidar_density = np.clip(bev_layers["lidar_density"], 0.0, 1.0)
    bev_layers["radar_expected_lidar_density_residual"] = np.maximum(
        radar_density - lidar_density,
        0.0,
    ).astype(np.float32)


def box_blur_3x3(grid: np.ndarray, iterations: int) -> np.ndarray:
    blurred = grid.astype(np.float32, copy=True)
    for _ in range(max(0, iterations)):
        padded = np.pad(blurred, 1, mode="edge")
        total = np.zeros_like(blurred, dtype=np.float32)
        for row_offset in range(3):
            for col_offset in range(3):
                total += padded[row_offset:row_offset + blurred.shape[0], col_offset:col_offset + blurred.shape[1]]
        blurred = total / 9.0
    return blurred.astype(np.float32)


def reliability_heatmap_target(
    clean: np.ndarray,
    faulty: np.ndarray,
    threshold: float,
    blur_iterations: int,
    ...
    pointwise_soft = np.clip(difference / max(threshold, 1e-6), 0.0, 1.0).astype(np.float32)
    binary_target = (difference > threshold).astype(np.float32)

    blurred = box_blur_3x3(pointwise_soft, blur_iterations)
    if np.max(blurred) > 0.0:
        blurred = blurred / np.max(blurred)

    regional_target = np.maximum(pointwise_soft, blurred).astype(np.float32)
    return regional_target, binary_target
...
def make_sample(index: int, scenes, args):
    ...
    add_radar_expected_lidar_density_residual(clean_layers)
    add_radar_expected_lidar_density_residual(faulty_layers)
    clean = stack_layers(clean_layers)
    faulty = stack_layers(faulty_layers)
    target, binary_target = reliability_heatmap_target(clean, faulty, args.target_threshold, args.heatmap_blur_iterations)
    ...
    metadata = {
        ...
        "target_type": "regional_reliability_heatmap",
        "binary_target_threshold": args.target_threshold,
        "heatmap_blur_iterations": args.heatmap_blur_iterations,
...
def main():
    parser = argparse.ArgumentParser(
        description=(
            "Create Model V5 samples with faults injected into raw Aeva LiDAR "
    ...
    print(f"V5 layers: {', '.join(V5_LAYERS)}")
    print("Fault injection: raw Aeva point cloud -> fault injector -> motion compensation -> BEV")
    print("Target: regional soft LiDAR unreliability heatmap plus binary target for metrics")
    print(f"Heatmap blur iterations: {args.heatmap_blur_iterations}")
