...
DEFAULT_MODEL_PATH = str(Path(FAST_OUTPUT_ROOT) / "models" / "Model_V3.pt")
DEFAULT_SAMPLE_DIR = str(Path(FAST_OUTPUT_ROOT) / "autoencoder_dataset")
DEFAULT_OUTPUT_DIR = str(Path(FAST_OUTPUT_ROOT) / "Model_V3_mask_predictions")
...
def load_model(model_path: Path, device):
    ...
    model = BEVFaultRestorationModelV3(
...
def main():
    parser = argparse.ArgumentParser(
        description="Visualize conditioned Model_V3 direct fault-mask predictions."
    )
    ...
    if args.sample is not None:
        predict_one(
            ...
            device,
            Path(args.sample),
            output_dir / f"{Path(args.sample).stem}_Model_V3_mask_overlay.png",
            threshold,
            channel_mean,
            channel_std,
            args.fault_type,
    ...
    for sample_path in sample_paths:
        output_path = output_dir / f"{sample_path.stem}_Model_V3_mask_overlay.png"
        ious.append(
            predict_one(
                model,
                device,
